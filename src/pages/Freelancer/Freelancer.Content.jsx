  ...
  return (
    <div className="mb-4 flex rounded-lg border-2 bg-ablack p-4 shadow-md">
      ...
      <img
        src={`http://localhost:3000/profilePhotos/${image}`}
        alt="Post Image"
        className="mt-2 h-20 border border-white"
      />
      <p className="text-sm text-white">Likes: {Object.keys(likes).length}</p>
      <button
        className={`flex items-center text-red-500 focus:outline-none ${
          isLiked ? "liked" : ""
        }`}
      ...
      <p className="text-sm text-white">Created At: {createdAt}</p> */}
    </div>
  );
};
